...
ERROS criar(Cliente clientes[], int *pos) {
  ...
  char tipo[TIPO]; // Definindo a variável tipo para armazenar a entrada do
                   // usuário
  ...
  do {
    ...
    if (tipo[0] == 'C' || tipo[0] == 'P') {
      ...
      printf(
          "Tipo de conta inválido. Escolha 'C' para Comum ou 'P' para Plus.\n");
...
ERROS debito(Cliente clientes[], int *pos) {
  ...
  for (i = 0; i < *pos; i++) {
    if (strcmp(clientes[i].cpf, cpf) ==
        0) { // Se encontrar o cliente com o CPF fornecido
      if (strcmp(clientes[i].senha, senha) ==
          0) { // Verifica se a senha está correta
        ...
        float saldo_negativo_maximo =
            (clientes[i].tipo_conta == COMUM)
                ? -1000.0f
        ...
        if (clientes[i].saldo - valor_debitado >= saldo_negativo_maximo) {
          ...
          return OK; // Retorna OK indicando que o débito foi realizado com
                     // sucesso
        } else {
          printf("Saldo insuficiente ou limite de saldo negativo excedido.\n");
          return SALDO_INSUFICIENTE; // Retorna erro indicando saldo
                                     // insuficiente ou limite de saldo negativo
                                     // excedido
        }
      } else {
...
ERROS deposito(Cliente clientes[], int *pos) {
  ...
  for (i = 0; i < *pos; i++) {
    if (strcmp(clientes[i].cpf, cpf) ==
      ...
      return OK; // Retorna OK indicando que o depósito foi realizado com
                 // sucesso
    }
  }
